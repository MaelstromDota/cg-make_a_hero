function OpenHeroSpells(heroname) {
	for (let i=0; i<$(`#SpellsHeroSelection`).GetChildCount(); i++) {
		...
		for (let j=0; j<attributepanel.GetChildCount(); j++) {
			const heropanel = attributepanel.GetChild(j);
	...
	const spells = CustomNetTables.GetTableValue("spells_info", heroname);
	const localplayer = Entities.GetLocalPlayer();
	for (let i=0; i<Object.keys(spells).length; i++) {
		const spell = Object.values(spells)[i];
		if (spell != "" && spell != "generic_hidden") {
			const spellpanel = $.CreatePanel("DOTAAbilityImage", $("#SpellsSpellSelection"), `AbilityPanel${spell}`, {class:"SpellsSpellIcon", hittest:"true", onactivate:`SelectSpell('${spell}')`, onmouseover:`ShowAbilityTooltip('${spell}')`, onmouseout:`HideAbilityTooltip('${spell}')`, abilityname:spell});
...
function InteractSpell(action) {
	for (let i=0; i<$("#SpellsSpellSelection").GetChildCount(); i++) {
		const spellpanel = $("#SpellsSpellSelection").GetChild(i);
...
function UpdateSpells() {
	...
	for (let i=0; i<35; i++) {
		const ability = Entities.GetAbility(localplayer, i);
...
const attributes_list = ["STRENGTH", "AGILITY", "INTELLECT", "ALL"];
function Init() {
	$(`#SpellsHeroSelection`).RemoveAndDeleteChildren();
	const heroes = CustomNetTables.GetTableValue("spells_info", "heroes");
	let heroes_sorted = [];
	let attributes = [];
	for (let i=0; i<Object.keys(heroes).length; i++) {
		const hero = Object.keys(heroes)[i];
		const info = Object.values(heroes)[i];
		heroes_sorted.push([hero, info]);
		const attribute = info["attribute"].substring(15);
		if (!attributes.includes(attribute)) {
			attributes.push(attribute);
		}
	}
	heroes_sorted.sort(function (a, b) {
		const _a = $.Localize(`#${a[0]}`).toLowerCase().replaceAll(" ", "");
		const _b = $.Localize(`#${b[0]}`).toLowerCase().replaceAll(" ", "");
	...
	for (let i=0; i<heroes_sorted.length; i++) {
		const hero = heroes_sorted[i][0];
		const attribute = heroes_sorted[i][1]["attribute"].substring(15);
...
function GetAbilityOwner(abilityname) {
	const spells = CustomNetTables.GetAllTableValues("spells_info");
	for (info of spells) {
		if (info["key"] != "heroes") {
