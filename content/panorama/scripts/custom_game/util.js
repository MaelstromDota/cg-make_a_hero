...
GameUI.GetDotaHud = function() {
	...
	try {
		while (!IsNull(temp_pan.GetParent())) {
...
GameEvents.SendEventToServer = function(event, data) {
	try {
		...
		if (!IsNull(temp_hud)) {temp_hud.DeleteAsync(0);};
...
Abilities.IsBehavior = function(find, behavior) {
	behavior = !IsNull(behavior) ? behavior : Abilities.GetBehavior(behavior);
	return parseInt(find & behavior) == parseInt(find);
};

Abilities.IsTargetType = function(find, type) {
	type = !IsNull(type) ? type : Abilities.GetAbilityTargetType(type);
	return parseInt(find & type) == parseInt(find);
};

Abilities.IsTargetFlag = function(find, flag) {
	flag = !IsNull(flag) ? flag : Abilities.GetAbilityTargetFlags(flag);
...
Entities.GetInventory = function(ent, start, stop) {
	start = IsNull(start) ? INVENTORY_SLOTS["SLOT_1"] : start;
	stop = IsNull(stop) ? INVENTORY_SLOTS["SLOT_6"] : stop;
	let inventory = [];
...
GameEvents.SubscribeEvent = function(eventName, func, unique) {
	GameUI.CustomUIConfig()["events"] = !IsNull(GameUI.CustomUIConfig()["events"]) ? GameUI.CustomUIConfig()["events"] : {}
	if (!IsNull(GameUI.CustomUIConfig()["events"][unique])) {
	...
	GameUI.CustomUIConfig()["events"][unique] = GameEvents.Subscribe(eventName, function(event) {
		if (!IsNull(event["encrypted_key"]) && event["encrypted_key"] != "" && event["encrypted_key"] == GameUI.CustomUIConfig()["encryption_keys"][event["encrypted_type"]]) {
