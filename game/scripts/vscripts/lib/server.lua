...
function CDOTA_PlayerResource:GetAvaiableNeutralItems(team)
	local items = {}
	for _tier, info in pairs(LoadKeyValues("scripts/npc/neutral_items.txt")) do
		local tier_time = string.split(table.keys(info["drop_rates"])[1], ":")
		if GameRules:GetDOTATime(false, false) >= (tonumber(tier_time[1]) * 60 + tonumber(tier_time[2])) then
			items[tonumber(_tier)] = table.keys(table.filter(info["items"], function(item, allowed) return tonumber(allowed) == 1 and (team == nil or not table.contains(NEUTRAL_ITEMS[team][tonumber(_tier)], item)) end))
		end
	end
	return items
end
function CDOTA_PlayerResource:IsNeutralItemAvaiable(itemname, team)
	return table.contains(table.open(self:GetAvaiableNeutralItems(team)), item_name)
end
function CDOTA_PlayerResource:RollNeutralItem(unit, team)
	local neutral_items = self:GetAvaiableNeutralItems(team)
	local infos = LoadKeyValues("scripts/npc/neutral_items.txt")
	local tiers = table.keys(table.filter(neutral_items, function(tier, items) return table.length(items) > 0 end))
	table.sort(tiers, function(a, b) return a < b end)
	for _, tier in pairs(tiers) do
		local chance = table.values(infos[tostring(tier)]["drop_rates"])[1][tostring(#NEUTRAL_ITEMS[team][tier])]
		if chance ~= nil and RollPseudoRandomPercentage(unit:IsAncient() and chance * 3 or chance, NEUTRAL_ITEMS_PSEUDORANDOM_ID, PlayerResource:GetSelectedHeroEntity(PlayerResource:GetPlayerIDsInTeam(team)[1])) then
			return table.choice(neutral_items[tier])
		end
	end
end
function CDOTA_PlayerResource:DropNeutralItem(itemname, point, hero)
	local tier = nil
	for _tier, info in pairs(LoadKeyValues("scripts/npc/neutral_items.txt")) do
		if table.contains(table.keys(table.filter(info["items"], function(item, allowed) return tonumber(allowed) == 1 end)), itemname) then
			tier = tonumber(_tier)
			break
		end
	end
	NEUTRAL_ITEMS[hero:GetTeamNumber()][tier] = NEUTRAL_ITEMS[hero:GetTeamNumber()][tier] or {}
	table.insert(NEUTRAL_ITEMS[hero:GetTeamNumber()][tier], itemname)
	return DropNeutralItemAtPositionForHero(itemname, point, hero, tier, true)
end
...
function CDOTA_BaseNPC:CreateCreepIllusions(owner, modifierKeys, numIllusions, padding, findClearSpace, issuperillusion)
	...
	for i=1, numIllusions do
		...
		for ability_slot = 0, DOTA_MAX_ABILITIES-1 do
