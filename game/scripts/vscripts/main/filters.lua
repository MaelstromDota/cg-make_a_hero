...
local function OrderSlotFilter(event, unit)
	...
	if event.order_type == DOTA_UNIT_ORDER_MOVE_ITEM and event.entindex_target and ability and ability:IsItem() then
		if (event.entindex_target == DOTA_ITEM_TP_SCROLL or ability:GetItemSlot() == DOTA_ITEM_TP_SCROLL) and (event.entindex_target ~= DOTA_ITEM_NEUTRAL_SLOT and ability:GetItemSlot() ~= DOTA_ITEM_NEUTRAL_SLOT) then
		...
		elseif (event.entindex_target == DOTA_ITEM_NEUTRAL_SLOT or ability:GetItemSlot() == DOTA_ITEM_NEUTRAL_SLOT) and unit:HasTalent("special_bonus_unique_meepo_7") then
			local other_slot_item = unit:GetItemInSlot(event.entindex_target)
			if (ability == nil or (not ability:IsDropsOnDeath())) and (other_slot_item == nil or (not other_slot_item:IsDropsOnDeath())) then
				if event.entindex_target == DOTA_ITEM_NEUTRAL_SLOT then
					ability:SetCanBeUsedOutOfInventory(true)
				elseif ability:GetItemSlot() == DOTA_ITEM_NEUTRAL_SLOT then
