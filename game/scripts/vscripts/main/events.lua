...
function CustomHeroArenaEvents:OnEntityKilled(event)
	...
	-- if npc:GetTeamNumber() == DOTA_TEAM_NEUTRALS and not npc:IsBoss() and not npc:IsControllableByAnyPlayer() and not attacker:HasModifier("modifier_teleporting_lua") then
	...
	-- 		if table.length(table.filter(FindUnitsInRadius(attacker:GetTeamNumber(), npc:GetAbsOrigin(), nil, 1200, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NOT_ILLUSIONS + DOTA_UNIT_TARGET_FLAG_INVULNERABLE + DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_OUT_OF_WORLD, FIND_CLOSEST, false), function(_, unit) return unit:IsTrueHero() end)) <= 0 then
	-- 			local neutral_item = PlayerResource:RollNeutralItem(npc, attacker:GetTeamNumber())
	-- 			if neutral_item ~= nil then
	-- 				PlayerResource:DropNeutralItem(neutral_item, npc:GetAbsOrigin(), attacker:IsTrueHero() and attacker or friends[1])
	-- 			end
	-- 		end
	-- 	end
	-- end
